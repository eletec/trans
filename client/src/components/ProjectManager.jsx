  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowProjects(false)}>
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-auto" onClick={e => e.stopPropagation()}>
        ...
        {currentProjectId && (
          <div className="flex items-center gap-2 mb-3 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
            <span className="text-sm text-blue-700 flex-1">
              Projet actif : <strong>{currentProjectName}</strong>
            </span>
            <button
        ...
        {saved && <div className="bg-green-50 text-green-700 p-2 rounded-lg text-sm mb-3">{saved}</div>}
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {projects.map(p => (
              <div key={p.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 ${p.id === currentProjectId ? 'bg-blue-50 ring-1 ring-blue-300' : 'bg-gray-50'}`}>
                ...
                <button
                  ...
                  title="Écraser avec les données actuelles"
